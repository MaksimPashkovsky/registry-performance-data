	...
	DWORD* Version) 
{
	...
	*Version = p->Version;
	...
	std::stringstream ss;
	// Retrieving system name (computer name)
	...
	WCHAR* Name)
{
	...
	*ParentObjectInstance = p->ParentObjectInstance;
	...
	else  // Convert the multi-byte instance name to Unicode
	{
		...
		if (FALSE == fSuccess)
		{
			// ConvertNameToUnicode for instance failed
			goto cleanup;
		}
	}
	...
	wchar_t* FinalValue) 
{
	...
	switch (CounterType0)
	{
		...
		case PERF_SAMPLE_FRACTION:
			...
			numerator = Data1 - Data0;
			denominator = Time1 - Time0;
			doubleValue = (double)(100.0 * numerator / denominator);
			ss << doubleValue << L"%";
			break;
		case PERF_COUNTER_TIMER_INV:
			// 100 * (1 - ((N1 - N0) / (D1 - D0)))
